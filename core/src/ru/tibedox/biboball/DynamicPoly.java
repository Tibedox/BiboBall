...
public class DynamicPoly {
    ...
    public DynamicPoly(World world, float x, float y) {
        ...
        chainShape.dispose(); // форму необходимо удалять
        body.setAngularVelocity(1);
    }

    void fromJson() {
        // read from json
        FileHandle file = Gdx.files.internal("star.json");
        ...
        for (int i = 0; i < chain.length; i++) {
            chain[i] = new Vector2(verts.vertices[i].x/100-1, verts.vertices[i].y/100-1);
